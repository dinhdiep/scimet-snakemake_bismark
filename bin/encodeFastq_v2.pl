...
sub encodeFastq {
    ...
    while(my $line1 = <FQ>){
        ...
        my @sequence = split "", $s;
        my @encoded = split "", $line2;
        my @encoded_qualities = split "", $line4;
        for(my $i = 0; $i < scalar(@sequence); $i++){
           if($sequence[$i] ne $encoded[$i]){
              my $update_q = ord($encoded_qualities[$i]) + 45;
              $encoded_qualities[$i] = chr($update_q); 
           }
        }
        $line4 = join("", @encoded_qualities);
        print FQ_OUT "$line1\n$line2\n$line3\n$line4\n";
    }
    close(FQ);
    close(FQ_OUT);
...
encodeFastq($ARGV[0], $ARGV[1], $ARGV[2], $ARGV[3]);
