...
sub main(){
    ...
    my $map_file = $ARGV[1];
    my $out_file = $ARGV[2];
    die("Unable to read chromosome sizes\n") if (!getChromSizes($fai_file));
    sortsam($map_file, $out_file);
    undef %chrSizes;
    my $total_mreads = 0;
...
sub sortsam{
    my $map_file = shift;
    my $out_file = shift;
    open(SAM_IN, "$map_file") || die("Error in opening $map_file.");
