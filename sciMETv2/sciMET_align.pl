...
if (defined $opt{'1'}) {
	...
	for ($round = 1; $round <= $r1_rounds; $round++) {
		...
		if ($round > 1) {
			...
			$prev_round = $round - 1;
			if (defined $opt{'k'}) {
                                $trim = "cutadapt --cores $threads -u $r1_trim -m $min_RL -o $opt{'O'}.trim_reads/$opt{'O'}.R1.$prev_round.trimmed.fq.gz $opt{'1'}";
				#$trim = "seqtk trimfq -b 0 -e $r1_trim $opt{'1'} > $opt{'O'}.trim_reads/$opt{'O'}.R1.$prev_round.trimmed.fq";
			} else {
                                $trim = "cutadapt --cores $threads -u $r1_trim -m $min_RL -o $opt{'O'}.trim_reads/$opt{'O'}.R1.$prev_round.trimmed.fq.gz $opt{'O'}.trim_reads/$opt{'O'}.R1.$prev_round.unmapped.fq.gz";
			...
			print LOG "\t$trim\n";
			system($trim);
			$in_reads = "$opt{'O'}.trim_reads/$opt{'O'}.R1.$prev_round.trimmed.fq.gz";
		} else {
			$in_reads = "$opt{'1'}";
		}
...
if (defined $opt{'2'}) {
	...
	for ($round = 1; $round <= $r2_rounds; $round++) {
		...
		if ($round > 1) {
			...
			$prev_round = $round - 1;
			if (defined $opt{'l'}) {
                                $trim = "cutadapt --cores $threads -u $r2_trim -m $min_RL -o $opt{'O'}.trim_reads/$opt{'O'}.R2.$prev_round.trimmed.fq.gz $opt{'2'}"
				#$trim = "seqtk trimfq -b 0 -e $r2_trim $opt{'2'} > $opt{'O'}.trim_reads/$opt{'O'}.R2.$prev_round.trimmed.fq";
			} else {
                                $trim = "cutadapt --cores $threads -u $r2_trim -m $min_RL -o $opt{'O'}.trim_reads/$opt{'O'}.R2.$prev_round.trimmed.fq.gz $opt{'O'}.trim_reads/$opt{'O'}.R2.$prev_round.unmapped.fq.gz" 
			...
			print LOG "\t$trim\n";
			system($trim);
			$in_reads = "$opt{'O'}.trim_reads/$opt{'O'}.R2.$prev_round.trimmed.fq.gz";
		} else {
			$in_reads = "$opt{'2'}";
		}
